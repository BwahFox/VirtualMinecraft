...
public class ComputerConfigScreen extends Screen {
	...
	protected void init() {
		...
		addRenderableWidget(Button.builder(Component.translatable("virtualminecraft.button.open_screen"), b -> {
			...
			if (be2 != null) {
				Minecraft.getInstance().gui.setScreen(new VmScreen(be2.getVmId(), cfg.name, pos));
			}
	...
	private void send(final VmControlPayload.Action action) {
		cfg = collect();
		if (ClientPlayNetworking.canSend(VmControlPayload.TYPE)) {
			ClientPlayNetworking.send(new VmControlPayload(pos, action, cfg));
		}
	}
	...
	public void extractRenderState(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final float a) {
		...
		graphics.text(this.font, title, (this.width - this.font.width(title)) / 2, 4, 0xFFFFFFFF);
		for (int i = 0; i < labels.size(); i++) {
			final int[] p = labels.get(i);
			graphics.text(this.font, labelText.get(i), p[0], p[1], 0xFFA0A0A0);
		}
		final ComputerBlockEntity be = blockEntity();
		...
		final int left = this.width / 2 - 150;
		graphics.text(this.font, this.font.plainSubstrByWidth(status, 300), left, statusY, color);
		...
		graphics.text(this.font, this.font.plainSubstrByWidth(disks.toString(), 300), left, disksY, 0xFFA0A0A0);
	}
}
