...
public final class InputSender {
	...
	public static void paste(final String text) {
		if (target == null || text == null || text.isEmpty() || !ClientPlayNetworking.canSend(dev.virtualminecraft.net.ScreenPastePayload.TYPE)) {
			return;
		}
		flush();
		ClientPlayNetworking.send(new dev.virtualminecraft.net.ScreenPastePayload(target, text));
	}
	...
	public static void worldPointer(final UUID screen, final int buttonMask, final int x, final int y) {
		if (screen == null || !ClientPlayNetworking.canSend(VmInputPayload.TYPE)) {
			return;
		}
		ClientPlayNetworking.send(new VmInputPayload(screen, List.of(VmInputPayload.Event.pointer(buttonMask, x, y))));
	}
	...
	public static void worldWheel(final UUID screen, final int rfbButtonBit, final int heldMask, final int x, final int y, final int notches) {
		if (screen == null || notches <= 0 || !ClientPlayNetworking.canSend(VmInputPayload.TYPE)) {
			return;
		}
		...
		ClientPlayNetworking.send(new VmInputPayload(screen, events));
	}
	...
	public static void worldChars(final UUID screen, final String text) {
		if (screen == null || text == null || text.isEmpty() || !ClientPlayNetworking.canSend(VmInputPayload.TYPE)) {
			return;
		}
	...
	private static void flush() {
		...
		if (!ClientPlayNetworking.canSend(VmInputPayload.TYPE)) {
			PENDING.clear();
			return;
		}
		ClientPlayNetworking.send(new VmInputPayload(target, new ArrayList<>(PENDING)));
		PENDING.clear();
	}
}
