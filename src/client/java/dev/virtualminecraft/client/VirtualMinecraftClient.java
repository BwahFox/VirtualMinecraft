...
public class VirtualMinecraftClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(ScreenInfoPayload.TYPE, (payload, context) -> ScreenTextures.onInfo(payload));
		ClientPlayNetworking.registerGlobalReceiver(ScreenRectPayload.TYPE, (payload, context) -> ScreenTextures.onRect(payload));
		ClientPlayNetworking.registerGlobalReceiver(dev.virtualminecraft.net.ScreenPalettePayload.TYPE, (payload, context) -> ScreenTextures.onPalette(payload));
		ClientPlayNetworking.registerGlobalReceiver(AudioPayload.TYPE, (payload, context) -> VmAudio.onPayload(payload));
		ClientPlayNetworking.registerGlobalReceiver(dev.virtualminecraft.net.ScreenCursorPayload.TYPE, (payload, context) -> ScreenTextures.onCursor(payload));
		ClientPlayNetworking.registerGlobalReceiver(ScreenTextPayload.TYPE, (payload, context) -> {
			final Minecraft mc = context.client();
		...
		VirtualMinecraft.clientHooks = new VirtualMinecraft.ClientHooks() {
			...
			public void openComputerScreen(final BlockPos pos) {
				...
				if (mc.level != null && mc.level.getBlockEntity(pos) instanceof ComputerBlockEntity) {
					mc.gui.setScreen(new ComputerConfigScreen(pos));
				}
			}
			...
			public void openMonitorScreen(final BlockPos sourcePos) {
				...
				if (mc.level != null && mc.level.getBlockEntity(sourcePos) instanceof ScreenSource source) {
					mc.gui.setScreen(new VmScreen(source.screenId(), source.screenName(), sourcePos));
				}
			}
		};
	}
