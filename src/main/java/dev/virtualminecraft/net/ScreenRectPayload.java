 ...
 */
public record ScreenRectPayload(UUID vm, int x, int y, int width, int height, int format, byte[] data) implements CustomPacketPayload {
	public static final Type<ScreenRectPayload> TYPE = new Type<>(VirtualMinecraft.id("screen_rect"));
	public static final StreamCodec<FriendlyByteBuf, ScreenRectPayload> CODEC = CustomPacketPayload.codec(ScreenRectPayload::write, ScreenRectPayload::new);
	...
	private ScreenRectPayload(final FriendlyByteBuf buf) {
	...
	@Override
	public Type<ScreenRectPayload> type() {
		return TYPE;
	}
}
