...
public class BridgeBlockEntity extends BlockEntity {
	...
	// handled by BridgeBlock.onRemove (1.20.1's affectNeighborsAfterRemoval), which fires only on a real break, and a stale entry is
	...
	public void load(final CompoundTag in) {
		super.load(in);
		pairId = null;
		Nbt.getString(in, "pair").ifPresent(s -> {
			try {
	...
	@Override
	protected void saveAdditional(final CompoundTag out) {
