...
public final class TextGrid {
	...
	public static TextGrid load(final CompoundTag in) {
		final TextGrid g = new TextGrid(Nbt.getIntOr(in, "cols", DEFAULT_COLS), Nbt.getIntOr(in, "rows", DEFAULT_ROWS));
		final int n = g.cols * g.rows;
		Nbt.getIntArray(in, "chars").ifPresent(a -> System.arraycopy(a, 0, g.chars, 0, Math.min(n, a.length)));
		Nbt.getIntArray(in, "fg").ifPresent(a -> System.arraycopy(a, 0, g.fg, 0, Math.min(n, a.length)));
		Nbt.getIntArray(in, "bg").ifPresent(a -> System.arraycopy(a, 0, g.bg, 0, Math.min(n, a.length)));
		g.cursorX = Nums.clamp(Nbt.getIntOr(in, "cursorX", 0), 0, g.cols - 1);
		g.cursorY = Nums.clamp(Nbt.getIntOr(in, "cursorY", 0), 0, g.rows - 1);
		g.curFg = Nbt.getIntOr(in, "curFg", DEFAULT_FG);
		g.curBg = Nbt.getIntOr(in, "curBg", DEFAULT_BG);
		return g;
	}
