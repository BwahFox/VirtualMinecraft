		...
		for (final ScreenViewers.Viewer v : viewers) {
			...
			if (wasResized || v.sentLod != lod) {
				...
				if (local) {
					...
					dev.virtualminecraft.net.ModNetworking.send(v.player, new ScreenInfoPayload(id, w, h, true, lod, flags));
				}
			}
			...
			if (v.needFull || palChanged) {
				if (lod == 0) {
					if (local) {
						...
						dev.virtualminecraft.net.ModNetworking.send(v.player, new ScreenPalettePayload(id, pal));
					}
				} else if (palChanged) {
					v.needFull = true;
				}
			...
			if (cw > 0 && (fresh || cshapeChanged || cmoved)) {
				...
				if (local) {
					...
					dev.virtualminecraft.net.ModNetworking.send(v.player, new ScreenCursorPayload(id, cx, cy, cvis, chx, chy, cw, ch, withShape ? cursorRgba : NO_SHAPE));
				}
				sent = true;
			}
			if (v.needFull) {
	...
	private boolean send(final ScreenViewers.Viewer v, final boolean local, final VirtualMinecraft.LocalBridge bridge, final List<Band> bands,
		...
		if (local) {
			...
			for (final ScreenRectPayload p : cache[lod]) {
				dev.virtualminecraft.net.ModNetworking.send(v.player, p);
			}
		}
