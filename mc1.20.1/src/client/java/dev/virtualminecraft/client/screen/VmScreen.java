...
public class VmScreen extends Screen {
	...
	public void render(final GuiGraphics graphics, final int mouseX, final int mouseY, final float a) {
		...
		if (tex != null) {
			...
			fitX = (availW - fitW) / 2;
			fitY = (availH - fitH) / 2;
			graphics.blit(tex.id, fitX, fitY, fitW, fitH, 0f, 0f, tex.width, tex.height, tex.width, tex.height);
			...
			if (cur != null && full[0] > 0 && full[1] > 0) {
				...
				if (cx1 > cx0 && cy1 > cy0) {
					...
					graphics.blit(cur.tex().id, gx0, gy0, Math.max(1, gx1 - gx0), Math.max(1, gy1 - gy0),
						cx0 - px0, cy0 - py0, cx1 - cx0, cy1 - cy0, cur.w(), cur.h());
				}
			}
		} else {
			fitW = 0;
			...
			graphics.drawString(this.font, msg, (this.width - this.font.width(msg)) / 2, this.height / 2 - 4, 0xFFFFFFFF);
		}
		final int[] full = ScreenTextures.fullSize(vm);
		...
		graphics.drawString(this.font, hint, (this.width - this.font.width(hint)) / 2, this.height - hintH + 2, HINT_COLOR);
	...
	public boolean keyPressed(final int key, final int scancode, final int modifiers) {
		final KeyEvent event = new KeyEvent(key, scancode, modifiers);
	...
	public boolean keyReleased(final int key, final int scancode, final int modifiers) {
		keys.keyUp(new KeyEvent(key, scancode, modifiers));
		return true;
	}

	@Override
	public boolean charTyped(final char codepoint, final int modifiers) {
		keys.charTyped(vm, new CharacterEvent(codepoint, modifiers));
		return true;
	}
	...
	public boolean mouseClicked(final double x, final double y, final int button) {
		final int bit = rfbButton(button);
		if (bit != 0) {
			InputSender.pointerButton(bit, true, fbX(x), fbY(y));
		}
		return true;
	}
	...
	public boolean mouseReleased(final double x, final double y, final int button) {
		final int bit = rfbButton(button);
		if (bit != 0) {
			InputSender.pointerButton(bit, false, fbX(x), fbY(y));
		}
		return true;
	}
	...
	/** 1.20.1 delivers vertical scrolling only; a horizontal wheel (RFB buttons 6/7) has no event to arrive on. */
	@Override
