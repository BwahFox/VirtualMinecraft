...
public final class WorldKeyboard {
	...
	public static boolean keyEvent(final KeyEvent event, final boolean down) {
		...
		if (options.keyJump.matches(event.key(), event.scancode()) || options.keyShift.matches(event.key(), event.scancode())) {
			if (down) {
				pendingBound = event;
				pendingBoundAt = net.minecraft.Util.getMillis();
	...
	public static boolean charEvent(final CharacterEvent event) {
		...
		if (pendingBound != null) {
			...
			if (net.minecraft.Util.getMillis() - pendingBoundAt <= BOUND_CONFIRM_MS) {
